...
d1join <- full_join(d1Y,d1F) #youth and families
d1final <- full_join(d1join,d1S) #join with stakeholders
rm(d1join,d1,d1F,d1S,d1Y)
...
d8S <- d8S %>% mutate_at(vars(9:11,25:27,35:37,47:49,59:61,75:77,90:92,111:113,123:125), #re-code all ADAPT ratings from string to numerical
            ~as.numeric(recode(.,
              "Limited Adaptation (2)"=2,
              "Full Adaptation (4)"=4,
              "No Adaptation (1)"=1,
...
d8S <- d8S %>% mutate_at(vars(12:14,28:30,38:40,50:52,62:64,78:80,93:95,114:116,126:128), #re-code all ACCESS ratings from string to numerical
            ~as.numeric(recode(.,
              "Limited Access (2)"=2,
              "Full Access (4)"=4,
              "No Access (1)"=1,
...
d8S <- d8S %>% select(-c(15,16,31,32,41,42,53,54,65,66,81,82,96,97,117,118,129,130)) #remove resource level and effectiveness ratings. They are not used in the model
